...
int check_year(char *date1, char *date2){
	int year1 = 0, year2 = 0;
	for (int i = 6; i <= 9; i++)
...
int check_month(char *date1, char *date2){
	int mon1 = 0, mon2 = 0;
	for (int i = 3; i <= 4; i++)
		mon1 = mon1 * 10 + date1[i] - '0';
	for (int i = 3; i <= 4; i++)
		mon2 = mon2 * 10 + date2[i] - '0';
...
int check_date(char *d1, char *d2){
	int date1 = 0, date2 = 0;
	for (int i = 0; i <= 1; i++)
		date1 = date1 * 10 + d1[i] - '0';
	for (int i = 0; i <= 1; i++)
		date2 = date2 * 10 + d2[i] - '0';
...
int older_date(char *date1, char *date2){
	//Function to compare years
	int year = check_year(date1, date2);
	printf("year %d\n", year);
	if (year == 1)//year1 > year2
		...
		int mon = check_month(date1, date2);
		if (mon == 1)// mon1 > mon2
			return 1;
			...
			int date = check_date(date1, date2);
